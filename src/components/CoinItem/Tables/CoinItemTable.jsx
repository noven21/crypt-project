...
const CoinItemTable = ({ coin, saveCoin, savedCoin }) => {
	return (
		<tr className='h-[80px] border-b overflow-hidden'>
			<td onClick={saveCoin}>{savedCoin ? <AiFillStar /> : <AiOutlineStar />}</td>
			<td>{coin.market_cap_rank}</td>
			<td>
				<Link href='coinpage' to={`/coin/${coin.id}`}>
			...
			<td className='w-[100px] hidden md:table-cell'>${coin.total_volume.toLocaleString()}</td>
			<td className='w-[100p] hidden sm:table-cell'>${coin.market_cap.toLocaleString()}</td>
			<td>
